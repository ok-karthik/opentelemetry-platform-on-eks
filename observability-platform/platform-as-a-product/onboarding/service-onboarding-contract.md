...
Go services should use a small internal telemetry package or copy the SDK bootstrap pattern from `workloads/apps-src/golang-app/telemetry.go`. Compiled services without SDKs can rely on the node-level eBPF (OBI) probe automatically.
