...
with Diagram(
    ...
    with Cluster("VPC 1: Workload VPC (10.0.0.0/16)", graph_attr=cluster_attr):
        ...
        with Cluster("EKS Workload Cluster (apps-workload-cluster-1)", graph_attr=cluster_attr):
            ...
            otel_agent = DaemonSet("OTel DaemonSet (:4317)\nk8sattributes + filelog")
            ...
            obi_ebpf >> Edge(color="purple", label="Loopback") >> otel_agent
    ...
    users >> alb
    otel_agent >> Edge(color="darkblue", label="OTLP / Gzip") >> vpc_peering >> nlb >> router
