...
- `dnsPolicy` is set explicitly because the agent still resolves `otel-gateway-regional.observability.svc.cluster.local` through CoreDNS while sharing the host network namespace.
