...
- **Sub-Millisecond In-Cluster Latency:** OTel DaemonSet agents stream directly to `otel-collector-tier2-router-collector.observability.svc.cluster.local:4317` via CoreDNS without traversing an external load balancer.
...
**Chosen:** Multi-window, multi-burn-rate SLO alerts ([`mimir-ruler-rules-configmap.yaml`](../observability-runtime/mimir-ruler-rules-configmap.yaml)) following the Google SRE Workbook pattern: four alerts per service pairing long windows with short windows (14.4x/6x/3x/1x against a 99.5% availability SLO). Fast-burn spikes page on-call via AWS Systems Manager Incident Manager (SSM Incident Manager); slow-burn budget consumption opens tickets on Alert-Sink.
